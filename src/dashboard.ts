...
export function productHtml(
  ...
  <main class="grid">
    ...
    <section class="card"><h2>Policy checks <span class="meta">${g?.policy_count ?? 0}</span></h2>${g?.policies?.map((p) => `<div class="item"><strong>${value(p.id)}</strong><p class="meta">v${value(p.version)} · ${value(p.when)} · ${value(p.action)}</p></div>`).join("") || empty("No policies declared. Organization-wide session policies may also apply.")}</section>
